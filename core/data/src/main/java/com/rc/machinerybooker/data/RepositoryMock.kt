...
class RepositoryMock @Inject constructor() : Repository {
    ...
    override fun observeMachineryOrderList(machineryOrderFilter: MachineryOrderFilter): Flow<List<MachineryOrder>> =
        flowOf(
            mockMachineryOrders
                .filter { machineryOrder ->
                machineryOrderFilter.id?.let {
                    machineryOrder.id == it
                } ?: true
            }
        )
    ...
    override fun getMachineryOrder(orderId: Long): MachineryOrder? =
        mockMachineryOrders.firstOrNull {
            it.id == orderId
        }
    ...
    override fun observeMachineryOrder(machineryOrderId: Long): Flow<MachineryOrder> =
        flow {
            mockMachineryOrders.firstOrNull { machineryOrder ->
                machineryOrder.id == machineryOrderId
            }?.let {
                emit(it)
            }
        }
